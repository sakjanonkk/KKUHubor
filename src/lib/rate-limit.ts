...
const RATE_LIMIT_RULES: { pattern: RegExp; config: RateLimitConfig; methods?: string[] }[] = [
  ...
  // Likes: 60 per minute
  { pattern: /^\/api\/likes$/, config: { maxRequests: 60, windowMs: 60 * 1000 } },
  // Search: 30 per minute
  { pattern: /^\/api\/search$/, config: { maxRequests: 30, windowMs: 60 * 1000 } },
  // Reports: 10 per hour
  { pattern: /^\/api\/reports$/, config: { maxRequests: 10, windowMs: 60 * 60 * 1000 }, methods: ["POST"] },
  // File uploads: 10 per hour
  { pattern: /^\/api\/summaries$/, config: { maxRequests: 10, windowMs: 60 * 60 * 1000 }, methods: ["POST"] },
  // Tag requests: 10 per hour
  { pattern: /^\/api\/tags$/, config: { maxRequests: 10, windowMs: 60 * 60 * 1000 }, methods: ["POST"] },
  // Course requests: 5 per hour
  { pattern: /^\/api\/requests$/, config: { maxRequests: 5, windowMs: 60 * 60 * 1000 }, methods: ["POST"] },
];

// Default: 60 requests per minute for any unmatched API route
const DEFAULT_CONFIG: RateLimitConfig = { maxRequests: 60, windowMs: 60 * 1000 };
